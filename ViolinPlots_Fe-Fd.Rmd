...
tmp.df2$OVF <- as.numeric(as.character(tmp.df2$OVF))*2.9889
#convert to the maximum value in algorith
tmp.df <- data.frame(OVF_df1500balanced$V1,"1500balanced")
names(tmp.df) <- c("OVF","TR")
tmp.df$OVF <- as.numeric(as.character(tmp.df$OVF))*2.9849
#convert to the maximum value in algorith
...
dp <- ggplot(OVF_dffe, aes(x=TR, y=OVF)) + 
  ...
  geom_jitter(height = 0, width = 0.1)+
  geom_boxplot(width=0.1, fill="white")+
  labs(title="Efficiency achieved in Genetic algorithm", y = "OVF")+ylab("Efficciency to measure cannonical HRF")
dp + theme_classic()
#individual dots
```
...
dp <- ggplot(tmp.df2, aes(x=TR, y=OVF)) + 
  ...
  geom_boxplot(width=0.1, fill="white")+
  geom_jitter(height = 0, width = 0.1)+
  labs(title="Efficiency achieved in Genetic algorithm (balanced)")+ylab("Efficciency to measure cannonical HRF")
dp + theme_classic()
